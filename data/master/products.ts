...
export async function fetchProducts(query: string, currentPage: number) {
  ...
  try {
    const productFind = await db.product.findMany({
      ...
      orderBy: [{ kategoriId: "asc" }, { updatedAt: "desc" }],
    });
...
export async function fetchProductPages(query: string) {
  ...
  try {
    ...
    const totalPagesProduct = Math.ceil(
      Number(productFind) / ITEMS_PER_PAGE_PRODUCT
    );
...
export async function fetchProductBynamePages(query: string) {
  ...
  try {
    ...
    const totalPagesProduct = Math.ceil(
      Number(totalProducts) / ITEMS_PER_PAGE_PRODUCT_NAME
    );
    return totalPagesProduct;
  } catch {
    console.error("Error Fetching Product");
...
export const fetchProductBySelect = async (
  brandId: string,
  groupId: string,
  categoryId: string,
  typeId: string
