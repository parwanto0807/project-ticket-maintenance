...
export async function generateTicketNumber() {
  ...
  try {
    ...
    const newTicketNumber = `T-${year}${month}${String(newIdNumber).padStart(
      4,
      "0"
    )}`;
...
export const fetchTicketListSchedule = async (
  query: string,
  currentPage: number
) => {
  noStore();
...
export const fetchTicketListHistory = async (
  query: string,
  currentPage: number
) => {
  noStore();
...
export const fetchTicketListHistoryTechnician = async (
  query: string,
  currentPage: number,
  email: string
) => {
...
export const fetchTicketListTechnician = async (
  query: string,
  currentPage: number,
  email: string
) => {
...
export const fetchTicketListPagesUser = async (query: string) => {
  ...
  try {
    ...
    const totalPagesTicket = Math.min(
      1,
      Math.ceil(ticketCount / ITEMS_PER_PAGE_TICKET)
    );
...
export const fetchTicketListPages = async (query: string) => {
  ...
  try {
    ...
    const totalPagesTicket = Math.ceil(ticketCount / ITEMS_PER_PAGE);
    return totalPagesTicket;
  } catch (error) {
    console.error("Error fetching ticket", error);
    throw new Error("Error fetching ticket");
...
export const fetchTicketAnalistAll = async () => {
  ...
  try {
    ...
    const filteredTickets = ticketFind.filter(
      (ticket) => new Date(ticket.createdAt) >= startDate
    );

    // 1️⃣ Grouping dan counting berdasarkan group.name
    const groupCounts = filteredTickets.reduce(
      (
        acc: { [key: string]: { total: number; latestCreatedAt: Date } },
        ticket
      ) => {
    ...
    const jenisProductCounts = filteredTickets.reduce(
      (
        acc: { [key: string]: { total: number; latestCreatedAt: Date } },
        ticket
      ) => {
        const jenisProductName =
          ticket.asset?.product?.jenisproduct?.name || "Unknown Product";
        const ticketDate = new Date(ticket.createdAt);
    ...
    const categoryCounts = filteredTickets.reduce(
      (
        acc: { [key: string]: { total: number; latestCreatedAt: Date } },
        ticket
      ) => {
        const groupName =
          ticket.asset?.product?.kategoriproduct?.name || "Unknown Group";
        const ticketDate = new Date(ticket.createdAt);
    ...
    const departmentCounts = filteredTickets.reduce(
      (
        acc: { [key: string]: { total: number; latestCreatedAt: Date } },
        ticket
      ) => {
        const departmentName =
          ticket.employee?.department?.dept_name || "Unknown Department";
        const ticketDate = new Date(ticket.createdAt);
    ...
    const result = {
      ...
      jenisProductData: Object.entries(jenisProductCounts).map(
        ([name, data]) => ({
          name,
          total: data.total,
          createdAt: data.latestCreatedAt,
      ...
      cetgoriProductData: Object.entries(categoryCounts).map(
        ([name, data]) => ({
          name,
          total: data.total,
          createdAt: data.latestCreatedAt,
      ...
      departmentTicketData: Object.entries(departmentCounts).map(
        ([name, data]) => ({
          name,
          total: data.total,
          createdAt: data.latestCreatedAt,
      ...
      (
        acc: {
          [key: string]: {
            ...
            employees: { name: string; ticketCount: number }[];
          };
        },
        ticket
      ) => {
        ...
        const existingEmployee = acc[deptName].employees.find(
