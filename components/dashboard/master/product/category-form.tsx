...
import { createCategory } from "@/action/master/product";
import { FormError } from "@/components/form-error";
import { FormSuccess } from "@/components/form-success";
import { KategoriProduct } from "@prisma/client";
import React, { useState, useTransition } from "react";
import { deleteCategory } from "@/action/master/product";
import { ScrollArea } from "@/components/ui/scroll-area";


const CategoryForm = ({ categoryFind }: { categoryFind: KategoriProduct[] }) => {
    const [error, setError] = useState<string | undefined>("");
    const [success, setSuccess] = useState<string | undefined>("");
    const [isPending, startTransition] = useTransition();
    const [selectedRow, setSelectedRow] = useState<string>('');
    const deleteWithId = (id: string) => deleteCategory(id);
    ...
    //         const unit = categoryFind.find(category => category.id === selectedRow);
    //         if (unit) {
    //             setInputName(unit.name);
    //             setInputNote(unit.note);
    //         }
    //     }
    // }, [selectedRow, categoryFind]); // Efek hanya dijalankan saat selectedRow atau unitFind berubah
    ...
    return (
        <Dialog>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle className="text-center mb-6">Add new category product</DialogTitle>
                    <Form {...form}>
                        <div className="w-full rounded-md mt-4 border items-center justify-center">
                            <form
                                    ...
                                    <div className="max-h-[270px] mt-2">
                                        <table className="min-w-full mt-0">
                                            ...
                                            <tbody className="border">
                                                {categoryFind?.map((category, index) => (
                                                    <tr key={category.id}
                                                        style={{
                                                    ...
                                                    >
                                                        ...
                                                        <td className=" whitespace-nowrap px-3 py-2">{category.name}</td>
                                                        <td className=" whitespace-nowrap px-3 py-2">{category.note}</td>
                                ...
                                <div className="relative">
                                    <Button className="absolute right-0" type="button" onClick={handleCategorySave}>Simpan</Button>
                                </div>
                            </form>
                        </div>
                    </Form>
