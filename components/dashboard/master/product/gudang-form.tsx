...
import { createGudang } from "@/action/master/product";
import { FormError } from "@/components/form-error";
import { FormSuccess } from "@/components/form-success";
import { Gudang } from "@prisma/client";
import React, { useState, useTransition } from "react";
import { deleteGudangWithId } from "@/action/master/product";
import { ScrollArea } from "@/components/ui/scroll-area";


const GudangForm = ({ gudangFind }: { gudangFind: Gudang[] }) => {
    const [error, setError] = useState<string | undefined>("");
    const [success, setSuccess] = useState<string | undefined>("");
    const [isPending, startTransition] = useTransition();
    const [selectedRow, setSelectedRow] = useState<string>('');
    const deleteWithId = (id: string) => deleteGudangWithId(id);
    ...
    //         const unit = gudangFind.find(category => category.id === selectedRow);
    //         if (unit) {
    //             setInputName(unit.name);
    //             setInputNote(unit.note);
    //         }
    //     }
    // }, [selectedRow, gudangFind]); // Efek hanya dijalankan saat selectedRow atau unitFind berubah
    ...
    const handleDelete = async (id: string) => {
        ...
        if (errorMessage) {
            ...
            setShowMessageDelete({ message: "Gudang deleted successfully!", visible: true });
            setTimeout(() => {
                setShowMessageDelete(prevState => ({ ...prevState, visible: false }));
            }, 2000);
        }
    ...
    return (
        <Dialog>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle className="text-center mb-6">Add new gudang product</DialogTitle>
                    <Form {...form}>
                            ...
                            <form
                                ...
                                <div className="w-full space-y-6 items-center justify-center">
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel>Gudang Name</FormLabel>
                                                <FormControl>
                                                    <Input
