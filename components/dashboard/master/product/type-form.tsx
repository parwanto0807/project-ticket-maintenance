...
import { createType } from "@/action/master/product";
import { FormError } from "@/components/form-error";
import { FormSuccess } from "@/components/form-success";
import { JenisProduct } from "@prisma/client";
import React, { useState, useTransition } from "react";
import { deleteType } from "@/action/master/product";
...
const TypeForm: React.FC<TypeFormProps> = ({ typeFind }) => {
    const [error, setError] = useState<string | undefined>("");
    const [success, setSuccess] = useState<string | undefined>("");
    const [isPending, startTransition] = useTransition();
    const [selectedRow, setSelectedRow] = useState<string>('');
    const deleteJenisProductWithId = (id: string) => deleteType(id);
    ...
    const handleDelete = async (id: string) => {
        const errorMessage = await deleteJenisProductWithId(id);
        if (errorMessage) {
            ...
            setShowMessageDelete({ message: "Type deleted successfully!", visible: true });
            setTimeout(() => {
                setShowMessageDelete(prevState => ({ ...prevState, visible: false }));
            }, 3000);
        }
    ...
    return (
        <Dialog>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle className="text-center mb-6">Add new type product</DialogTitle>
                    <Form {...form}>
