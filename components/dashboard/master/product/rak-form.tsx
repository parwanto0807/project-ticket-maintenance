...
import { RakSchema } from "@/schemas";
import { Input } from "@/components/ui/input"
import { Button } from "@/components/ui/button"
import { TrashIcon, PlusIcon } from "@heroicons/react/24/outline"
import { useForm } from "react-hook-form"
import { createRak } from "@/action/master/product";
import { FormError } from "@/components/form-error";
import { FormSuccess } from "@/components/form-success";
import { Rak } from "@prisma/client";
import React, { useState, useTransition } from "react";
import { deleteRakWithId } from "@/action/master/product";
import { ScrollArea } from "@/components/ui/scroll-area";


const RakForm = ({ rakFind }: { rakFind: Rak[] }) => {
    const [error, setError] = useState<string | undefined>("");
    const [success, setSuccess] = useState<string | undefined>("");
    const [isPending, startTransition] = useTransition();
    const [selectedRow, setSelectedRow] = useState<string>('');
    const deleteWithId = (id: string) => deleteRakWithId(id);
    const [showMessageDelete, setShowMessageDelete] = useState<{ message: string; visible: boolean }>({ message: "", visible: false });


    // 1. Define your form.
    const form = useForm<z.infer<typeof RakSchema>>({
    ...
    const onSubmit = (values: z.infer<typeof RakSchema>) => {
        setError("");
        setSuccess("");
    ...
    return (
        <Dialog>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle className="text-center mb-6">Add new rak product</DialogTitle>
                    <Form {...form}>
                            ...
                            <form
                                ...
                                <div className="w-full space-y-6 items-center justify-center">
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormLabel>Rak Kolom & Baris</FormLabel>
                                                <FormControl>
                                                    <Input
