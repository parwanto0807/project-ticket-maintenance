...
import { deleteGroup } from "@/action/master/product";
import { createGroup } from "@/action/master/product";
...
const GroupForm = ({ groupFind }: { groupFind: Group[] }) => {
    const [error, setError] = useState<string | undefined>("");
    const [success, setSuccess] = useState<string | undefined>("");
    const [isPending, startTransition] = useTransition();
    const [selectedRow, setSelectedRow] = useState<string>('');
    const deleteWithId = (id: string) => deleteGroup(id);
    ...
    //         const unit = groupFind.find(category => category.id === selectedRow);
    //         if (unit) {
    //             setInputName(unit.name);
    //             setInputNote(unit.note);
    //         }
    //     }
    // }, [selectedRow, groupFind]); // Efek hanya dijalankan saat selectedRow atau unitFind berubah
    ...
    return (
        <Dialog>
            ...
            <DialogContent className="w-full max-w-2xl">
                <DialogHeader>
                    <DialogTitle className="text-center mb-6">Add new group product</DialogTitle>
                    <Form {...form}>
                        <div className="w-full rounded-md mt-4 border items-center justify-center">
                            <form
                                        ...
                                        <table className="min-w-full mt-0">
                                            ...
                                            <thead className="rounded-lg text-left text-xs font-normal sticky top-0 bg-background">
                                                <tr>
                                                    ...
                                                    <th scope="col" className="px-5 py-5 font-medium items-center">Category Name</th >
                                            ...
                                            <tbody className="border">
                                                {groupFind?.map((data, index) => (
                                                    ...
                                                    >
                                                        <td className="whitespace-normal break-words px-3 py-3">{index + 1}</td>
                                                        <td className="whitespace-wrap text-wrap px-3 py-4">{data.name}</td>
                                                        <td className="whitespace-wrap break-words px-3 py-3">{data.note}</td>
                                                        <td className="whitespace-normal px-3 py-3"><TrashIcon onClick={() => handleDelete(data.id)} className="text-red-500 w-8 rounded-md border p-2 hover:bg-gray-800" /></td>
                                                    </tr >
                                                ))}
                                ...
                                <div className="relative">
                                    <Button className="absolute right-0" type="button" onClick={handleSaveGroup}>Simpan</Button>
                                </div>
                            </form>
                        </div>
                    </Form>
