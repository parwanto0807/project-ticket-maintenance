        ...
        <div className="relative overflow-hidden bg-gradient-to-br from-blue-700 via-blue-600 to-blue-800 p-6 pb-16 pt-10">
          ...
          <div className="relative flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-blue-100 text-xs font-medium uppercase tracking-widest">{getTimeBasedGreeting()}</p>
          ...
          <div className="relative bg-white/10 backdrop-blur-lg rounded-2xl p-4 border border-white/20 shadow-xl">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-blue-100 text-[10px] font-bold uppercase tracking-tighter">{t("your_specialization")}</p>
                <h2 className="text-lg font-bold text-white tracking-tight leading-tight">{data?.technician?.specialization || t("general_technician")}</h2>
              </div>
              <div className="flex flex-col items-end">
                ...
                <p className="text-white/60 text-[9px] mt-1 italic uppercase tracking-tighter">{t("verified_provider")}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Card className="border-none shadow-xl shadow-blue-900/5 overflow-hidden group hover:scale-[1.02] transition-transform duration-300">
              <CardContent className="p-4 bg-white">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-xl bg-blue-50 text-blue-600 transition-colors group-hover:bg-blue-600 group-hover:text-white duration-300">
                    <TrendingUp className="w-5 h-5" />
                  </div>
                  <div>
                    <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">{t("total_tasks")}</p>
            ...
            <Card className="border-none shadow-xl shadow-orange-900/5 overflow-hidden group hover:scale-[1.02] transition-transform duration-300">
              <CardContent className="p-4 bg-white">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-xl bg-orange-50 text-orange-600 transition-colors group-hover:bg-orange-600 group-hover:text-white duration-300">
                    <Clock className="w-5 h-5 transition-transform group-hover:rotate-12" />
                  </div>
                  <div>
                    <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">{t("in_progress")}</p>
                    <p className="text-2xl font-black text-slate-800">{data?.stats?.inProgress || 0}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            <Card className="border-none shadow-xl shadow-red-900/5 overflow-hidden group hover:scale-[1.02] transition-transform duration-300">
              <CardContent className="p-4 bg-white">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-xl bg-red-50 text-red-600 transition-colors group-hover:bg-red-600 group-hover:text-white duration-300">
                    <AlertCircle className="w-5 h-5" />
                  </div>
                  <div>
                    <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">{t("pending")}</p>
                    <p className="text-2xl font-black text-slate-800">{data?.stats?.pending || 0}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            <Card className="border-none shadow-xl shadow-green-900/5 overflow-hidden group hover:scale-[1.02] transition-transform duration-300">
              <CardContent className="p-4 bg-white">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-xl bg-green-50 text-green-600 transition-colors group-hover:bg-green-600 group-hover:text-white duration-300">
                    <CheckCircle2 className="w-5 h-5 transition-transform group-hover:scale-110" />
                  </div>
                  <div>
                    <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">{t("completed")}</p>
                    <p className="text-2xl font-black text-slate-800">{data?.stats?.completed || 0}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-0.5">
                            <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full tracking-tighter shadow-sm">{task.ticketNumber}</span>
                            <span className="text-[9px] font-bold text-slate-400">{format(new Date(task.updatedAt), "HH:mm")}</span>
                          </div>
                          ...
                          <p className="text-[10px] text-slate-500 font-medium truncate italic tracking-tighter mb-1">“{task.troubleUser || t("no_problem_summary")}”</p>
                          <div className="flex items-center gap-3">
                            <div className="flex items-center gap-1">
                              <div className="w-1.5 h-1.5 rounded-full bg-slate-200"></div>
                              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">{t(task.status as TranslationKeys) || task.status}</span>
                            </div>
                            <div className="flex items-center gap-1">
                              <div className="w-1.5 h-1.5 rounded-full bg-slate-200"></div>
                              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter truncate max-w-[80px]">{task.asset?.location || t("no_location")}</span>
                            </div>
                          </div>
                        </div>
                        <ChevronRight className="w-4 h-4 text-slate-300 group-hover:text-blue-500 group-hover:translate-x-1 transition-all mt-5" />
                      </div>
                    </CardContent>
                  </Card>
                </Link>
            ...
            <CardContent className="p-6 relative">
              <div className="flex items-center justify-between gap-4">
                <div className="flex-1">
                  <h3 className="text-white font-black text-sm uppercase tracking-wider mb-1">{t("offline_access")}</h3>
                  <p className="text-slate-400 text-[10px] font-medium leading-relaxed tracking-tight italic">{t("install_app_desc")}</p>
                </div>
